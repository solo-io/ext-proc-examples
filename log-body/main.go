...
func (s *server) Process(srv service_ext_proc_v3.ExternalProcessor_ProcessServer) error {
	...
	for {
		...
		case *service_ext_proc_v3.ProcessingRequest_RequestHeaders:
			log.Printf("Got RequestHeaders (not currently handled)")
		...
		case *service_ext_proc_v3.ProcessingRequest_RequestBody:
			log.Printf("Got RequestBody")

			body := req.Request.(*service_ext_proc_v3.ProcessingRequest_RequestBody)
			log.Printf("Logging Request Body Chunk")
			log.Printf("Body chunk: %s\n", string(body.RequestBody.Body))
			log.Printf("EndOfStream: %v\n", body.RequestBody.EndOfStream)
			resp.Response = &service_ext_proc_v3.ProcessingResponse_RequestBody{}

		case *service_ext_proc_v3.ProcessingRequest_ResponseBody:
			log.Printf("Got ResponseBody")

			body := req.Request.(*service_ext_proc_v3.ProcessingRequest_ResponseBody)
			log.Printf("Logging Response Body Chunk")
			log.Printf("Body chunk: %s\n", string(body.ResponseBody.Body))
			log.Printf("EndOfStream: %v\n", body.ResponseBody.EndOfStream)
		...
		case *service_ext_proc_v3.ProcessingRequest_ResponseHeaders:
			log.Printf("Got ResponseHeaders (not currently handled)")
